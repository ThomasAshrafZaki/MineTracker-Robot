...
================================================================================
  ...
  Author  : Robot Team
  Version : 4.2 — Environment Classification removed entirely
================================================================================
"""
...
def draw_hud(
    ...
    #   - تصنيف البيئة بالكامل (Environment classification) — أتشال
    #     نهائيًا من كل النظام (vision_processor.py لم يعد يحسبه خالص)
    #
...
def parse_args():
    ...
    p.add_argument("--no-mine",      action="store_true",
                   help="Disable secondary mine-detection model (best.pt). "
                        "Enabled by DEFAULT — pass this flag to turn it off.")
    p.add_argument("--mine-autostop", action="store_true",
                   help="Also send a stop signal when mine model fires (use only after you trust the model)")
    return p.parse_args()
...
def main():
    args = parse_args()
    # ── v4.2: الموديل الثانوي (best.pt) بقى مفعّل افتراضياً ──────────
    # قبل كده كان لازم تكتب --use-mine يدوي كل مرة. دلوقتي العكس:
    # شغال تلقائي، ولو عايز تعطّله صراحة اكتب --no-mine.
    # سيبنا args.use_mine زي ما هي عشان باقي main.py (وفيه استخدامات
    # كتير ليها تحت) يكمل يشتغل بنفس الاسم من غير أي تعديل تاني.
    args.use_mine = not args.no_mine
    ...
    log.info(f"  sign_detector={not args.no_sign}")
    log.info(f"  mine_detector={args.use_mine} (default ON unless --no-mine)  mine_autostop={args.mine_autostop}")
    log.info("=" * 60)
    ...
    try:
        while running[0]:
            ...
            # ── mine detector gate (اختياري — testing) ───────
            # بشكل افتراضي: detection + عرض على الشاشة + تسجيل في
            # mission_logger (صورة + Excel row في logs/mine_detections).
            # لازم --mine-autostop صريحة عشان يبعت إشارة إيقاف فعلية —
            # كده تقدر تجرب وتتأكد من دقة الموديل من غير ما الروبوت
            # ياخد قرار حركة عليه، لكن كل كشف بيتسجل في اللوج عشان
            # تراجعه بعدين وتشوف كام false-positive حصل.
            if args.use_mine:
                mine = vision.get_latest_mine()
                if mine is not None and mine.found() and mine.is_valid(max_age=1.0):
                    if args.mine_autostop:
                        bridge.send_sign_stop()  # نفس إشارة الإيقاف المستقلة المستخدمة لكشف اللوحات

                    # v4.2: تسجيل فعلي في mission_logger — فيه cooldown
                    # داخلي (MINE_COOLDOWN_S) فمش هيسبام حتى لو نادينا
                    # عليه كل تيك واللغم لسه ظاهر قدام الكاميرا.
                    best_mine = max(mine.boxes, key=lambda b: b["conf"])
                    logger.log_mine_detection(
                        label=best_mine["label"],
                        confidence=best_mine["conf"],
                    )
